    ...
    try {
        ...
        if (isUserHasCart) {
            ...
            for (let i = 0; i < isUserHasCart.items.length; i++) {
                if (isUserHasCart.items[i].productId == productId) {
                    flag = true
                    if (quantity != null) {
                        isUserHasCart.items[i].quantity = isUserHasCart.items[i].quantity + quantity
                    }
                    else { isUserHasCart.items[i].quantity = isUserHasCart.items[i].quantity + 1 }
                    ...
                    if (quantity != null) {
                        updateCart['totalPrice'] = isUserHasCart.totalPrice + (price.price * quantity)
                    }
                    else { updateCart['totalPrice'] = isUserHasCart.totalPrice + price.price }
            ...
            if (flag == false) {
                ...
                if (quantity != null) {
                    const items = [{ productId, quantity: quantity }]
                    updateCart = { $addToSet: { items: items } }
                    updateCart['$inc'] = {}
                    updateCart['$inc']['totalPrice'] = price.price * quantity
        ...
        if (!isUserHasCart) {
            ...
            if (quantity != null) {
                ...
                return res.status(201).send({ status: true, message: "Product is added in cart", data: createdCart })
            }
            else {
                const newCart = {
                    userId: userId,
...
const getCartData = async (req, res) => {
    try {
        ...
        return res.status(200).send({ status: true, message: "User's Cart details", data: CartDetails })
    }
    catch (err) {
        console.log(err)
        return res.status(500).send({ status: false, message: err.message })
